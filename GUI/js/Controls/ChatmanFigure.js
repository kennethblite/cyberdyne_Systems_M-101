     ...
     this.chatmanFigureContainer.find('[clickCommand]')
         .click(function(e) {
            ...
            if ( e.button == 0 && command ) {
                executeCommand(figure.name, command, '');
            }
         });
...
ChatmanFigure.prototype.createAnimation = function(animationData) {
    ...
    var animateFunc = function() {
        setTimeout(function() {            
            ...
            while (i != animationData.length && animationData[i].S == currTime) {
                figureControl.animate(animationData[i].C, animationData[i].P);
                ++i;
            }
